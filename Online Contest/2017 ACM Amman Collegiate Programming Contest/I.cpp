...
ll n;
bool check(int x){
    int lineSize=(n-x+1)/x;
//    cout<<lineSize<<endl;
    for(int i=1;i<x;i++){
//        cout<<(i-1)*(lineSize+1)+lineSize<<" "<<line[(i-1)*(lineSize+1)+lineSize]<<endl;
        if(line[(i-1)*(lineSize+1)+lineSize]!=' ') return false;
    }
//    cout<<endl;
    return true;
}
...
//        cout<<n<<endl;
        bool flag=false;
        int len=Num.Prime.size();
        for(int i=0;i<len;i++){
            int segment=Num.Prime[i];
            if(segment>=n) break;
            if((n-segment+1)%segment) continue;
            if(check(segment)){
                flag=true;
                cout<<"YES"<<endl;
                break;
