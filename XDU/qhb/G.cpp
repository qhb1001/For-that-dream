...
int main() {
	NumberTheory number(100005);
	int T;
	ios::sync_with_stdio(false);
	cin.tie(0);
	long long int now;

	cout << number.Miller_Rabin(1e17 + 7) << endl;
	// for(int i = 0; i < number.Prime.size(); ++i)
	// 	cout << number.Prime[i] << endl;
	// cin >> T;
	// while(T--) {
	// 	cin >> now;
	// 	if(number.IsPrime(now)) {
	// 		cout << now - 1 << endl;
	// 		continue;
	// 	}
	// 	vector<long long int> q = number.DecomposingFactor(now);
	// 	long long int sum = 0;
